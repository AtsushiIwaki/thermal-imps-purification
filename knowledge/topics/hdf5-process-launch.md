...
On macOS and GNU/Linux, its [native launcher](../../src/git_process/posix.rs) addresses two distinct descriptor
...
The macOS helper sets `POSIX_SPAWN_CLOEXEC_DEFAULT`; GNU/Linux uses
`posix_spawn_file_actions_addclosefrom_np` after duplicating stdout, requiring glibc >=2.34.
Both wrap `posix_spawnp` in `hdf5_metno::sync::sync`, using the shared HDF5 mutex.
They release that mutex before draining
...
Two macOS/GNU-Linux [public regressions](../../src/git_process/tests.rs) cover the boundaries:
...
The [GNU/Linux investigation](../../docs/investigations/2026-09-08-linux-hdf5-spawn.md) records
the CI failure and real descriptor/reopen reproduction. This knowledge page adds no measurement.
Other targets, including Linux musl and Windows, retain standard process launching.
Independently launched subprocesses and native operations bypassing the
